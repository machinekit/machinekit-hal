...
//#define SR_OUTPUT_FREQ          0x2102                   // 0.01Hz units
#define ST_EMERGENCY_STOPPED    0x0021          // EF1/ESTOP

#define SR_MOTOR_SPEED          0x2207          // RPM
...
int write_data(modbus_t *ctx, haldata_t *haldata, param_pointer p)
{
    ...
    if (!*(haldata->enabled)) {
        ...
        if (modbus_write_register(ctx, REG_COMMAND1, 0) < 0) {
            ...
            (*haldata->errorcount)++;
            rtapi_print_msg(RTAPI_MSG_ERR, "Error writing REG_COMMAND1 register");
            p->last_errno = errno;
            return errno;
        }
        return 0;
...
int read_initial(modbus_t *ctx, haldata_t *haldata, param_pointer p)
{
    ...
    *(haldata->max_rpm) = *(haldata->upper_limit_hz) * 
    ...
    p->last_errno = errno;
    (*haldata->errorcount)++;
    rtapi_print_msg(RTAPI_MSG_ERR, "Error reading initial registers 0x%4.4x: %s", curr_reg, modbus_strerror(errno));
    if (p->debug)
...
//    static int pollcount = 0;
    ...
    if (!p->read_initial_done) {
        if ((retval = read_initial(ctx, haldata, p))) {
            rtapi_print_msg(RTAPI_MSG_ERR, "read_data failed while reading initial registers");
            return retval;
        } else {
...
/*
    ...
    *(haldata->status) = status_reg;
    ...
    hal_float_t hzcalc = haldata->motor_nameplate_RPM / haldata->motor_nameplate_hz;

    GETREG(SR_OUTPUT_FREQ, &freq_reg);
    *(haldata->freq_out) = freq_reg * 0.01;
    *(haldata->RPM) = freq_reg * .01 * hzcalc;
    *(haldata->RPS) = *(haldata->RPM)/60;
...
//        *(haldata->RPS) = val/60.0;
    ...
    p->last_errno = errno;
    (*haldata->errorcount)++;
    rtapi_print_msg(RTAPI_MSG_ERR, "Error2 reading registers 0x%4.4x: %s", curr_reg, modbus_strerror(errno));
    if (p->debug)
...
int main(int argc, char **argv)
{
    ...
    if( modbus_rtu_set_serial_mode(p->ctx, MODBUS_RTU_RS485) != 0) {
        fprintf(stderr, "%s: ERROR: modbus_rtu_set_serial_mode(%s): %s\n", p->progname, p->device, modbus_strerror(errno) );
        goto finish;
    }

/*
    if( modbus_rtu_set_rts(p->ctx, MODBUS_RTU_RTS_DOWN) != 0) {
        fprintf(stderr, "%s: ERROR: modbus_rtu_set_rts(%s): %s\n", p->progname, p->device, modbus_strerror(errno) );
        goto finish;
    }

    if( modbus_rtu_set_rts_delay(p->ctx, 1) != 0) {
        fprintf(stderr, "%s: ERROR: modbus_rtu_set_rts_delay(%s): %s\n", p->progname, p->device, modbus_strerror(errno) );
        goto finish;
    }
    */
