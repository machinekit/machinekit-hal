...
class TestClass(object):
    ...
    def test_pin_creation_after_ready(module_fixture):
        try:
    ...
    def test_pins_dictionary(self):
        assert len(hal.pins) == 1
        assert fqpname in hal.pins
        assert hal.pins[fqpname].name == fqpname


    def test_pin_initial_value(self):
        assert c[pname] == 42


    def test_pin_value_assignment(self):
        c[pname] = 4711
        assert c[pname] == 4711
    ...
    def test_pin_attributes(self):
        ...
        assert abs(p.epsilon - hal.epsilon[epsindex]) < 0.0001
        assert p.handle > 0
    ...
    def test_signals_dictionary(self):
        print("hal.signals", hal.signals)
        assert len(hal.signals) == 1
        assert signame in hal.signals
        assert hal.signals[signame].name == signame


    def test_ccomp_and_epsilon(self):
